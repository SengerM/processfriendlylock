...
SEPARATION_STRING = '<-asd->' # This just has to be a very weird string that will never appear in the name of whoever wants to acquire the lock.

def _check_name(name:str):
	if not isinstance(name, str):
		raise TypeError(f'`name` must be a string.')
	if SEPARATION_STRING in name:
		raise ValueError(f'`name` cannot contain {repr(SEPARATION_STRING)}, please use another.')
